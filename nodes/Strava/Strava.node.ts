...
export class Strava implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Strava',
		name: 'strava',
		icon: { light: 'file:../../icons/strava.svg', dark: 'file:../../icons/strava.dark.svg' },
		group: ['input'],
		version: 1,
		...
		properties: [
			...
			},
			{
				...
				options: [
					...
					{
						name: 'List Comments',
						value: 'listActivityComments',
						action: 'List comments by activity ID',
					},
					{
						name: 'List Kudos',
						value: 'listActivityKudoers',
						action: 'List kudoers by activity ID',
					},
					{
						name: 'List Logged-In Athlete Activities',
						value: 'listAthleteActivities',
						action: 'List logged in athlete activities',
					},
			...
			},
			{
				...
				options: [
					...
					{
						name: 'List by Segment',
						value: 'listSegmentEfforts',
						action: 'List segment efforts by segment ID',
					},
				],
			...
			},
			{
				...
				options: [
					{ name: 'Get', value: 'getClub', action: 'Get a club by ID' },
					{
						...
						value: 'listClubActivities',
						action: 'List club activities by ID',
					},
					{ name: 'List Admins', value: 'listClubAdmins', action: 'List club admins by ID' },
					{
						name: 'List Logged-In Athlete Clubs',
						value: 'listAthleteClubs',
						action: 'List logged in athlete clubs',
					},
			...
			},
			{
				...
				options: [
					...
					{
						name: 'List by Athlete',
						value: 'listRoutesByAthlete',
						action: 'List routes by athlete ID',
					},
				],
				default: 'getRoute',
			},
			...
			},
			{
				...
				default: 70,
				required: true,
				displayOptions: { show: { operation: ['updateLoggedInAthlete'] } },
			},
			{
				displayName: 'Name',
				name: 'name',
			...
			},
			{
				...
				default: 'Run',
				required: true,
				description: 'Strava sport type (for example Run, Ride, Walk)',
				displayOptions: { show: { operation: ['createActivity'] } },
			},
			{
				...
				description: 'Legacy type field accepted by Strava',
				displayOptions: { show: { operation: ['createActivity'] } },
			},
			{
				...
				default: '',
				required: true,
				description: 'ISO 8601 local date-time, e.g. 2018-02-16T14:52:54',
				displayOptions: { show: { operation: ['createActivity', 'listSegmentEfforts'] } },
			},
			{
			...
			},
			{
				...
				displayOptions: { show: { operation: ['listSegmentEfforts'] } },
			},
			{
				...
				type: 'boolean',
				default: false,
				displayOptions: { show: { operation: ['getActivity'] } },
			},
			{
				displayName: 'Before (Unix Seconds)',
				name: 'before',
				type: 'number',
				default: 0,
				description: 'Only return activities before this Unix timestamp in seconds',
				displayOptions: { show: { operation: ['listAthleteActivities'] } },
			},
			{
				displayName: 'After (Unix Seconds)',
				name: 'after',
				type: 'number',
				default: 0,
				description: 'Only return activities after this Unix timestamp in seconds',
				displayOptions: { show: { operation: ['listAthleteActivities'] } },
			},
			{
			...
			},
			{
				...
				default: true,
				required: true,
				displayOptions: { show: { operation: ['starSegment'] } },
			},
			{
			...
			{
				displayName: 'Southwest Latitude',
				name: 'sw_lat',
				type: 'number',
			...
			{
				displayName: 'Southwest Longitude',
				name: 'sw_lng',
				type: 'number',
				typeOptions: { numberPrecision: 6 },
				default: -122.5,
			...
			{
				displayName: 'Northeast Latitude',
				name: 'ne_lat',
				type: 'number',
				typeOptions: { numberPrecision: 6 },
				default: 37.9,
			...
			{
				displayName: 'Northeast Longitude',
				name: 'ne_lng',
				type: 'number',
				typeOptions: { numberPrecision: 6 },
				default: -122.3,
			...
			},
			{
				...
				default: 0,
				required: true,
				displayOptions: { show: { operation: ['listRoutesByAthlete'] } },
			},
			{
				...
				default: 0,
				required: true,
				displayOptions: { show: { operation: ['getUpload'] } },
			},
			{
				displayName: 'Gear ID',
				name: 'gearId',
				...
				default: '',
				required: true,
				displayOptions: { show: { operation: ['getGear'] } },
			},
			{
				...
				default: 'data',
				required: true,
				displayOptions: { show: { operation: ['exportRouteGpx', 'exportRouteTcx'] } },
			},
			{
				displayName: 'Upload Binary Property',
			...
			},
			{
				...
				displayOptions: {
					show: {
						operation: ['getActivityStreams', 'getSegmentEffortStreams', 'getSegmentStreams'],
					},
				},
				options: [
			...
			},
			{
				...
				default: false,
				displayOptions: { show: { operation: paginationOperations } },
			},
			{
				displayName: 'Limit',
				name: 'limit',
				...
				displayOptions: { show: { operation: paginationOperations, returnAll: [false] } },
			},
			{
				displayName: 'Page',
				name: 'page',
			...
			try {
				...
				if (resource === 'athlete') {
					if (operation === 'getLoggedInAthlete')
						responseData = (await stravaApiRequest.call(this, 'GET', '/athlete')) as IDataObject;
					if (operation === 'getAthleteStats')
						responseData = (await stravaApiRequest.call(
							this,
							'GET',
							`/athletes/${this.getNodeParameter('id', itemIndex) as number}/stats`,
						)) as IDataObject;
					if (operation === 'updateLoggedInAthlete')
						responseData = (await stravaApiRequest.call(this, 'PUT', '/athlete', {
							form: { weight: this.getNodeParameter('weight', itemIndex) as number },
						})) as IDataObject;
					if (operation === 'getLoggedInAthleteZones')
						responseData = (await stravaApiRequest.call(
							this,
							'GET',
							'/athlete/zones',
				...
				if (resource === 'activity') {
					...
					if (operation === 'createActivity') {
						const additional = this.getNodeParameter(
							'createActivityAdditionalFields',
							itemIndex,
						) as IDataObject;
						responseData = (await stravaApiRequest.call(this, 'POST', '/activities', {
							form: {
								...
								sport_type: this.getNodeParameter('sport_type', itemIndex) as string,
								start_date_local: this.getNodeParameter('start_date_local', itemIndex) as string,
								elapsed_time: this.getNodeParameter('elapsed_time', itemIndex) as number,
								...(typeof additional.commute === 'boolean'
									? { commute: additional.commute ? 1 : 0 }
									: {}),
								...(typeof additional.trainer === 'boolean'
									? { trainer: additional.trainer ? 1 : 0 }
									: {}),
							},
						})) as IDataObject;
					}
					if (operation === 'getActivity')
						responseData = (await stravaApiRequest.call(this, 'GET', `/activities/${id}`, {
							qs: {
								include_all_efforts: this.getNodeParameter(
									'include_all_efforts',
									itemIndex,
								) as boolean,
							},
					...
					if (operation === 'updateActivity')
						responseData = (await stravaApiRequest.call(this, 'PUT', `/activities/${id}`, {
							body: this.getNodeParameter('updateActivityFields', itemIndex) as IDataObject,
						})) as IDataObject;
					if (operation === 'getActivityLaps')
						responseData = (await stravaApiRequest.call(
							this,
							'GET',
							`/activities/${id}/laps`,
						)) as IDataObject[];
					if (operation === 'getActivityZones')
						responseData = (await stravaApiRequest.call(
							this,
							'GET',
							`/activities/${id}/zones`,
					...
					if (operation === 'listAthleteActivities') {
						...
						responseData = (this.getNodeParameter('returnAll', itemIndex) as boolean)
							...
							: (
									(await stravaApiRequest.call(this, 'GET', '/athlete/activities', {
										qs: {
											...qs,
								...
								).slice(0, this.getNodeParameter('limit', itemIndex) as number);
					}
					if (operation === 'listActivityComments' || operation === 'listActivityKudoers') {
						const perPage = this.getNodeParameter('per_page', itemIndex) as number;
						const endpoint =
							operation === 'listActivityComments'
								? `/activities/${id}/comments`
								: `/activities/${id}/kudos`;
						const additional =
							operation === 'listActivityComments'
								? (this.getNodeParameter(
										'activityCommentsAdditionalFields',
										itemIndex,
									) as IDataObject)
								: {};
						responseData = (this.getNodeParameter('returnAll', itemIndex) as boolean)
							...
							: (
									(await stravaApiRequest.call(this, 'GET', endpoint, {
										qs: {
											page: this.getNodeParameter('page', itemIndex) as number,
											per_page: Math.min(
												this.getNodeParameter('limit', itemIndex) as number,
				...
				if (resource === 'segment') {
					...
					if (operation === 'listStarredSegments') {
						...
						responseData = (this.getNodeParameter('returnAll', itemIndex) as boolean)
							...
							: (
									(await stravaApiRequest.call(this, 'GET', '/segments/starred', {
										qs: {
											page: this.getNodeParameter('page', itemIndex) as number,
											per_page: Math.min(
												this.getNodeParameter('limit', itemIndex) as number,
					...
					if (operation === 'starSegment')
						responseData = (await stravaApiRequest.call(this, 'PUT', `/segments/${id}/starred`, {
							form: { starred: this.getNodeParameter('starred', itemIndex) as boolean },
						})) as IDataObject;
					if (operation === 'exploreSegments') {
						const additional = this.getNodeParameter(
							'segmentExploreAdditionalFields',
							itemIndex,
						) as IDataObject;
						...
						responseData = (await stravaApiRequest.call(this, 'GET', '/segments/explore', {
							qs: {
								bounds,
								activity_type: this.getNodeParameter('activity_type', itemIndex) as string,
								...additional,
							},
						})) as IDataObject;
					}
				...
				if (resource === 'segmentEffort') {
					if (operation === 'getSegmentEffort')
						responseData = (await stravaApiRequest.call(
							this,
							'GET',
							`/segment_efforts/${this.getNodeParameter('id', itemIndex) as number}`,
						)) as IDataObject;
					if (operation === 'listSegmentEfforts') {
						const perPage = this.getNodeParameter('per_page', itemIndex) as number;
						const qs: IDataObject = {
							segment_id: this.getNodeParameter('segment_id', itemIndex) as number,
							...(this.getNodeParameter('start_date_local', itemIndex)
								? {
										start_date_local: this.getNodeParameter(
											'start_date_local',
											itemIndex,
										) as string,
									}
								: {}),
							...(this.getNodeParameter('end_date_local', itemIndex)
								? { end_date_local: this.getNodeParameter('end_date_local', itemIndex) as string }
								: {}),
						};
						responseData = (this.getNodeParameter('returnAll', itemIndex) as boolean)
							? await stravaApiRequestAllItems.call(this, '/segment_efforts', qs, perPage)
							: (
									(await stravaApiRequest.call(this, 'GET', '/segment_efforts', {
										qs: {
											...qs,
											page: this.getNodeParameter('page', itemIndex) as number,
											per_page: Math.min(
				...
				if (resource === 'club') {
					if (operation === 'getClub')
						responseData = (await stravaApiRequest.call(
							this,
							'GET',
							`/clubs/${this.getNodeParameter('id', itemIndex) as number}`,
						)) as IDataObject;
					if (operation === 'listAthleteClubs') {
						...
						responseData = (this.getNodeParameter('returnAll', itemIndex) as boolean)
							...
							: (
									(await stravaApiRequest.call(this, 'GET', '/athlete/clubs', {
										qs: {
											page: this.getNodeParameter('page', itemIndex) as number,
											per_page: Math.min(
												this.getNodeParameter('limit', itemIndex) as number,
					...
					if (['listClubMembers', 'listClubAdmins', 'listClubActivities'].includes(operation)) {
						...
						const endpointMap: Record<string, string> = {
							listClubMembers: `/clubs/${id}/members`,
							listClubAdmins: `/clubs/${id}/admins`,
							listClubActivities: `/clubs/${id}/activities`,
						};
						const perPage = this.getNodeParameter('per_page', itemIndex) as number;
						responseData = (this.getNodeParameter('returnAll', itemIndex) as boolean)
							? await stravaApiRequestAllItems.call(this, endpointMap[operation], {}, perPage)
							: (
									(await stravaApiRequest.call(this, 'GET', endpointMap[operation], {
										qs: {
											page: this.getNodeParameter('page', itemIndex) as number,
											per_page: Math.min(
												this.getNodeParameter('limit', itemIndex) as number,
				...
				if (resource === 'gear')
					responseData = (await stravaApiRequest.call(
						this,
						'GET',
						`/gear/${this.getNodeParameter('gearId', itemIndex) as string}`,
					)) as IDataObject;
				...
				if (resource === 'route') {
					if (operation === 'getRoute')
						responseData = (await stravaApiRequest.call(
							this,
							'GET',
							`/routes/${this.getNodeParameter('id', itemIndex) as number}`,
						)) as IDataObject;
					if (operation === 'listRoutesByAthlete') {
						...
						responseData = (this.getNodeParameter('returnAll', itemIndex) as boolean)
							...
							: (
									(await stravaApiRequest.call(this, 'GET', `/athletes/${id}/routes`, {
										qs: {
											page: this.getNodeParameter('page', itemIndex) as number,
											per_page: Math.min(
												this.getNodeParameter('limit', itemIndex) as number,
					...
					if (operation === 'exportRouteGpx' || operation === 'exportRouteTcx') {
						...
						const binaryData = await stravaDownloadAsBinary.call(
							this,
							`/routes/${id}/export_${extension}`,
							`route-${id}.${extension}`,
							operation === 'exportRouteGpx'
								? 'application/gpx+xml'
								: 'application/vnd.garmin.tcx+xml',
						);
						returnData.push({
							json: { id, format: extension },
							binary: {
								[this.getNodeParameter('binaryPropertyName', itemIndex) as string]: binaryData,
							},
							pairedItem: { item: itemIndex },
						});
						continue;
				...
				if (resource === 'upload') {
					if (operation === 'createUpload') {
						...
						const fileName =
							(items[itemIndex].binary?.[binaryProperty]?.fileName as string | undefined) ??
							`upload.${dataType}`;
					...
					if (operation === 'getUpload')
						responseData = (await stravaApiRequest.call(
							this,
							'GET',
							`/uploads/${this.getNodeParameter('uploadId', itemIndex) as number}`,
						)) as IDataObject;
				}

				if (resource === 'stream') {
					const id = this.getNodeParameter('id', itemIndex) as number;
					if (operation === 'getRouteStreams')
						responseData = (await stravaApiRequest.call(
							this,
							'GET',
							`/routes/${id}/streams`,
					...
					if (
						['getActivityStreams', 'getSegmentEffortStreams', 'getSegmentStreams'].includes(
							operation,
						)
					) {
						const endpointMap: Record<string, string> = {
							getActivityStreams: `/activities/${id}/streams`,
							getSegmentEffortStreams: `/segment_efforts/${id}/streams`,
							getSegmentStreams: `/segments/${id}/streams`,
						};
						responseData = (await stravaApiRequest.call(this, 'GET', endpointMap[operation], {
							qs: {
								keys: (this.getNodeParameter('keys', itemIndex) as string[]).join(','),
								key_by_type: this.getNodeParameter('key_by_type', itemIndex) as boolean,
							},
						})) as IDataObject;
					}
				}
